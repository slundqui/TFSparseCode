    ...
    def buildModel(self, inputShape):
        ...
        with tf.device(self.device):
            ...
            with tf.name_scope("conv1"):
                #Max pooled values
                self.W_conv1= weight_variable_xavier([3, 3, inputShape[2], inputShape[2]], "conv1_w", False)
                self.B_conv1= bias_variable([inputShape[2]], "conv1_b")
                self.pool1 = tf.nn.max_pool(self.input, ksize=[1, 4, 4, 1], strides=[1, 4, 4, 1], padding='SAME', name="pooled")
                self.h_conv1 = tf.nn.conv2d(self.pool1, self.W_conv1, [1, 1, 1, 1], padding="SAME", name="h_conv1")

            with tf.name_scope("conv2"):
                weightShape = (inputShape[2]*4, self.numClasses) #4 for pooling output
                #Max pooled values
                self.W_slp = weight_variable_xavier(weightShape, "slp_w", False)
                self.B_slp = bias_variable([self.numClasses], "slp_b")
                self.fcpool = tf.nn.max_pool(self.h_conv1, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME', name="pooled")
                self.flat_pooled = tf.reshape(self.fcpool , [-1, 4*inputShape[2]])
        ...
        tf.histogram_summary('pool1', self.pool1, name="pooled")
        tf.histogram_summary('pool2', self.flat_pooled, name="pooled")
        tf.histogram_summary('gt', self.gt, name="gt")
        #Conv layer histograms
        tf.histogram_summary('h_conv1', self.h_conv1, name="h_conv1")
        tf.histogram_summary('est', self.est, name="est")
        #Weight and bias hists
        tf.histogram_summary('conv1_w', self.W_conv1, name="conv1_w")
        tf.histogram_summary('conv1_b', self.B_conv1, name="conv1_b")
